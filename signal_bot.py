...
# ===================== VERSION =====================
VERSION = "V7.0 Smart Trader (structural TP/SL, MTF brain, single-position discipline)"
...
# Спреды/буферы и структурные правила
SPREAD_BUFFER   = {"NG": 0.0020, "XAU": 0.20, "BTC": 5.0}
TP_EXTRA_BUFFER = {"NG": 0.0100, "XAU": 0.30, "BTC": 15.0}  # чтобы TP был «реальным»
SL_MIN_GAP      = {"NG": 0.0040, "XAU": 0.25, "BTC": 12.0}  # минимальная дистанция SL от входа
...
# Порог уверенности/торгов
CONF_MIN_IDEA   = 0.25   # идеи мягче
CONF_MIN_TRADE  = 0.55   # боевой сигнал

# RR-фильтры (мягкие). Можно поставить 0.0, если хочешь совсем убрать.
RR_TRADE_MIN    = 1.50
RR_MIN_IDEA     = 1.20

# фильтр «свежести» (не шлём сигнал, если цена уже убежала дальше этого лимита от entry)
FRESH_MULT      = 10.0
...
# Сессии (UTC) — чисто для скоринга (не блок)
...
def choose_structural_target(entry: float, side: str, symbol: str,
    ...
    if side == "BUY":
        ...
        for gap, anchor, tf in ordered:
            ...
            if tp_candidate <= entry: continue
...
def momentum_confirmation(df1m: pd.DataFrame, symbol: str, side: str, atr15: float):
    ...
    atr_ratio = 0.0 if atr15 <= 0 else impulse / atr15
    ...
    reason = f"Импульс подтверждён {aligned}/4 свечами 1m, движение ≈ {abs_move} ({'вверх' if side=='BUY' else 'вниз'})"
...
def build_setup(df1m: pd.DataFrame, symbol: str, tf_label: str, dxy_bias: str | None = None):
    ...
    frames = {"1m": df1m, "5m": df5, "15m": df15, "60m": df60, "240m": df240}
    if not df1440.empty:
        frames["1440m"] = df1440
    ...
    if fvg_ok:
        ...
        score += 20; score_breakdown.append(("FVG M15", 20)); core_signals += 1
    ...
    if choch_ok:
        ...
        score += 18; score_breakdown.append(("CHOCH M5", 18)); core_signals += 1
    ...
    if sweep15:
        ...
        score += 14; score_breakdown.append(("Liquidity sweep", 14)); core_signals += 1
    ...
    score += 12; score_breakdown.append(("Structural SL", 12))
    reasons.append(
    ...
    score += 14; score_breakdown.append(("Structural TP", 14))
    ...
    score += 15; score_breakdown.append(("Momentum confirm", 15)); core_signals += 1

    last15 = df15.iloc[-2]
    if fib_ote_ok(float(last15["Open"]), float(last15["Close"]), entry):
        reasons.append("Цена в зоне OTE 62–79% на M15")
        score += 9; score_breakdown.append(("OTE", 9))
    if fvg_ok:
    ...
    if rr >= RR_TRADE_MIN:
        ...
        score += 10; score_breakdown.append(("Risk/Reward", 10))

    if core_signals < 2:  # минимум 2 ядра сигнала
        return None
    ...
    return {
        ...
        "reasons": reasons, "score_breakdown": score_breakdown, "core_signals": core_signals,
        "atr15": atr15, "momentum_ratio": momentum_ratio, "momentum_aligned": momentum_aligned,
        "dxy_bias": dxy_bias,
    }
...
async def handle_symbol(session: aiohttp.ClientSession, symbol: str, dxy_df: pd.DataFrame | None):
    ...
    # Если не AUTO — обрабатываем только выбранный символ
    ...
    # дисциплина: не открываем новую позицию, если где-то уже открыто
    if any(trade.values()):
        return
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            try:
                ...
                # AUTO = NG+XAU; BTC только если выбран режим «биток»
