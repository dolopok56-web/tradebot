...
# ===================== CONFIG =====================
SYMBOLS = {
    "NG":  {"name": "Natural Gas", "tf": "M5", "yahoo": "NG%3DF"},
    "XAU": {"name": "Gold",        "tf": "M5", "yahoo": "XAUUSD%3DX"},
}
...
def dynamic_buffer(symbol, df, _): return SPREAD_BUFFER.get(symbol, 0.0)

def _resample(df, minutes):
    # df с равномерным мин. шагом -> OHLC нужного ТФ
...
def _had_liquidity_sweep(df, lookback=20):
    i = len(df) - 2
    hh = _swing_high(df, lookback); ll = _swing_low(df, lookback)
...
def _choch(df, want):
    ...
    return c[-2] > max(c[-9:-2]) if want=="UP" else c[-2] < min(c[-9:-2])
...
def _is_consolidation_break(df):
    ...
    i=len(df)-2; window=df.iloc[i-12:i]
...
def _inside_higher_ob(df_low, df_high):
    ...
    cl=float(df_low["Close"].iloc[-2]); body=df_high.iloc[-2]
    top=max(body["Open"],body["Close"]); bot=min(body["Open"],body["Close"])
    return bot<=cl<=top

def _fib_ote_ok(a,b,price):
    if a==b: return False
    lo,hi=sorted([a,b])
    lvl62=hi-0.62*(hi-lo); lvl79=hi-0.79*(hi-lo)
    loZ,hiZ=min(lvl62,lvl79),max(lvl62,lvl79)
    return loZ<=price<=hiZ

# ===================== Market data (Yahoo v8) =====================
async def _get_yahoo(session, ticker: str) -> pd.DataFrame:
    ...
    try:
        ...
        df = pd.DataFrame({"Open":q["open"],"High":q["high"],"Low":q["low"],"Close":q["close"]}, index=ts)
        df = df.dropna().tail(600).reset_index(drop=True)
        return df
    except Exception:
        return pd.DataFrame()

async def get_df(session, symbol: str) -> pd.DataFrame:
    if symbol == "NG":   return await _get_yahoo(session, SYMBOLS["NG"]["yahoo"])
    if symbol == "XAU":  return await _get_yahoo(session, SYMBOLS["XAU"]["yahoo"])
    if symbol == "DXY":  return await _get_yahoo(session, "DX-Y.NYB")  # индекс доллара
    return pd.DataFrame()

# ===================== DXY Bias для золота =====================
async def get_dxy_bias(session):
    try:
        df = await get_df(session,"DXY")
        if df.empty: return None
        df60 = _resample(df,60); df240 = _resample(df,240)
...
def build_setup(df1m, symbol, tf_label, dxy_bias=None):
    ...
    df5=_resample(df1m,5); df15=_resample(df1m,15); df60=_resample(df1m,60); df240=_resample(df1m,240)
    ...
    last15=df15.iloc[-2]
    if _fib_ote_ok(float(last15["Open"]),float(last15["Close"]),entry): score+=10
    if sweep15: score+=10
    if fvg15:
        fvg_w=abs(fvg15_top-fvg15_bot)
        rng=(df15["High"]-df15["Low"]).tail(20).mean()
        if pd.notna(rng) and rng>0 and fvg_w>=1.5*float(rng): score+=7
...
def format_signal(setup, buffer):
    ...
    return (f"🔥 {side} {SYMBOLS[sym]['name']} | {tf}\n"
            f"TP: {rnd(sym,setup['tp'])} | SL: {rnd(sym,setup['sl'])}\n"
            f"Entry: {rnd(sym,setup['entry'])}  Buf≈{rnd(sym,buffer)}  "
    ...
    while True:
        try:
            ...
            msg=(f"[ALIVE] NG: {rnd('NG',c_ng)}, ATR15: {rnd('NG',a_ng)} | "
                 f"XAU: {rnd('XAU',c_xau)}, ATR15: {rnd('XAU',a_xau)}. Status: OK.")
            await log_bot.send_message(OWNER_ID,msg)
        except Exception as e:
            try:    await log_bot.send_message(OWNER_ID,f"[ALIVE ERROR] {e}")
            except: pass
...
async def main_loop():
    while True:
        try:
            ...
            try:    await log_bot.send_message(OWNER_ID,f"[ERROR] {e}")
            except: pass
